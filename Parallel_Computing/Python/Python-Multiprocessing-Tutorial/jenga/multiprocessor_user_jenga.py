...
def run_jenga_game(game_number):
    game = Jenga()
    game.play_game()
    return f"Game {game_number} completed"
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool() as pool:
        pool.map(run_jenga_game, range(total_games))

    end_time = datetime.now()
    total_time = (end_time - start_time).total_seconds()

    print(f"Total time taken for {total_games} games (Multiprocessing): {total_time} seconds")
